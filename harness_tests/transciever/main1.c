...
void trans_uart_rx_cb_test(void){
    ...
    //buffer cleared if calling trans_cmd_resp_avail() results in trans_cmd_resp_avail,
    //but then after it's set back to false.. check seperately the clear buffer then.

    //either trans_cmd_resp_avail was false to begin, or it was true->cleared buffer->false
    ASSERT_EQ(trans_cmd_resp_avail,false); //should be reset to false this quickly?
}
...
void trans_scw_test(void){
    ...
    uint8_t test3 = 0;
    uint8_t value = 0;
    //let's test all cases bc yep! (probs unneccessary but.. hehe :)
    for (uint8_t bit_index=0; bit_index<15; bit_index++){
        for (value = 0; value <= 1; value++){
            scw = scw_initial;
            test3 = set_trans_scw_bit(bit_index,value);
            ASSERT_EQ(test3,1);
            if (value==0){
                ASSERT_EQ(scw,scw_initial & (~_BV(bit_index)));
            } else if (value==1){
                ASSERT_EQ(scw,scw_initial | (~_BV(bit_index)));
            }
        }
...
void trans_beacon_period_test(void){
    uint16_t period = 0;
    uint8_t test = set_trans_beacon_period(period);
    ASSERT_EQ(test,1);
    uint8_t rssi = 0;
    period = 0;
...
void trans_call_sign_test(void){
    ...
    ASSERT_EQ(test,1);
    //what about what happens to call_sign? well, then call_sign takes on
    //values previosuly in the buffer... any use testing for that? ehhh see about that..
    //buffer cleared...
...
void get_trans_num_packets_test(void){
    ...
    get_trans_num_rx_packets_crc(&rssi, &num_rx_packets_crc);
    ASSERT_EQ(test,1);
}
...
test_t t7 = {.name = "swc_bits_test", .fn = swc_bits_test };
test_t t8 = {.name = "trans_freq_test", .fn = trans_freq_test };
test_t t9 = {.name = "trans_pipe_timeout_test", .fn = trans_pipe_timeout_test };
test_t t10 = {.name = "trans_beacon_period_test", .fn = trans_beacon_period_test };
test_t t11 = {.name = "trans_call_sign_test", .fn = trans_call_sign_test };
test_t t12 = {.name = "get_uptime_test", .fn = get_uptime_test };
test_t t13 = {.name = "get_trans_num_packets_test", .fn = get_trans_num_packets_test };
